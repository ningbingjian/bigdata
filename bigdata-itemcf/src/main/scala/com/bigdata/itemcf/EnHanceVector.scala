...
object EnHanceVectors extends Serializable{
  def topK(vector:SparseVector,k:Int): SparseVector = {
      val (indices, values) = {
        val zipped = vector.indices zip vector.values
        zipped.sortBy(_._2).reverse.take(k).unzip
      }
      Vectors.sparse(Integer.MAX_VALUE, indices.toArray, values.toArray).asInstanceOf[SparseVector]
  }
